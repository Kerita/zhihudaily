<template>
  <div>
	<group>
		<cell v-for='theme in themes' :title='theme.name'  :inline-desc='theme.description' v-link="{path : '/theme/'+theme.id+'/'+getIndex(themes,theme)+'/'+theme.name+'/'+theme.description}">
		<img :src='urls[getIndex(themes,theme)]' width='100px' height='70px' style='padding-left: 30px'>
		</cell>
	</group>
</div>
</template>
...
export default {
  ...
  data () {
    return {
      ...
      url: 'index.php/index/getdata/getThemeList',
      urls: []
    }
  },
  methods: {
  ...
  ready () {
    document.title = '主题日报'
    for (var i = 0; i < 12; i++) {
      this.urls.push('http://ob0818c80.bkt.clouddn.com/theme' + i + '.jpg')
    }
  }
}
</script>
